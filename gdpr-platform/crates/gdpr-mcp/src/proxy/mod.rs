...
//! 2. `spawn_blocking` → `EnginePool::anonymize_batch_strings` (L1 regex + L2 ONNX).
...
pub async fn chat_completions(
    ...
    if !raw_texts.is_empty() {
        ...
        let cleaned: Vec<String> = tokio::task::spawn_blocking(move || {
            pool.anonymize_batch_strings(&cloned.iter().map(String::as_str).collect::<Vec<_>>())
        })
        .await
        .unwrap_or_else(|e| {
    ...
    // 4. Forward to TensorZero, stream response verbatim (SSE ok)
    match state
        .http_client
        ...
        .await
    {
        Ok(resp) => {
            ...
            // Buffer full response body then rehydrate pseudo-tokens.
