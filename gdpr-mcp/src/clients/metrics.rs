//! Prometheus metrics for the GDPR-MCP server.
//!
//! All metrics are registered with the default global registry on first use.
//! Call [`gather_text`] to produce a Prometheus text exposition (e.g. for a
...
/// Return the process-global [`Metrics`] instance, registering on first call.
///
/// Panics only if two calls race during the very first initialisation and the
/// metric names collide in the default registry — which cannot happen with
/// `OnceLock` (only one closure ever runs).
...
///
/// Returns an empty string on encoding failure (non-fatal — scrapes will just
/// see no data rather than an error).
