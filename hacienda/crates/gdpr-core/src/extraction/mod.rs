...
///
/// When the `ocr` feature is enabled, this delegates to kreuzberg.
/// Without the feature, only plain text files are supported.
...
pub async fn extract_document_bytes(
    ...
    #[cfg(feature = "ocr")]
    {
        ...
        return Ok(ExtractionOutput { content: result.content, quality_score: 1.0 });
    }
    #[cfg(not(feature = "ocr"))]
    {
        let _ = mime_type;
